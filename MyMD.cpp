...
static void Periodic(Mol& mol, double length) {
	// This function applies periodic boundary condition to the atomic coordinates.
	for (int i = 0; i < 1000; i++) {
		mol.x[i] = fmod(mol.x[i],length);
		mol.y[i] = fmod(mol.y[i],length);
		mol.z[i] = fmod(mol.z[i],length);
	}
}
...
static void WriteCoord(Mol& mol, string filename, int traj, int status) {
	...
	filename += ".gro";
	if (traj == 0) {
		coord.open(filename.c_str(), ios::out);
	}
	else {
		if (traj == 2) {
			coord.open(filename.c_str(), ios::out);
		...
		coord.open(filename.c_str(), ios::app);
	}
	if (status == -100) {
		coord << "Initial Coordinates" << endl << "  1000" << endl;
	}
	else if (status == -99) {
		coord << "Result Coordinates" << endl << "  1000" << endl;
	}
	else {
		coord << "Step: " << status << endl << "  1000" << endl;
	}
	for (int i = 0; i < 1000; i++) {
...
int main() {
	...
	for (int step = 1; step <= StepNum; step++) { //start simulation
		...
		if (iftraj == true && step % 40 == 0) { // output trajectory file if wanted
			if (step == 40 ) {
				WriteCoord(mol, "traj", 2, step); //if it is the first step, clear traj file first
			}
			else {
				WriteCoord(mol, "traj", 1, step); //if it is not the first step,append to the previous file
			}
		}
	...
	cout << "Simulation successfully ended!!!"<<endl;
	WriteCoord(mol, "output", 0, -99); //generating output coordinates
	return 0;
}